...
| `/whisper stats` | Print `stats` and read it back in 3-4 lines. Lead with `does_it_help`: the whispered arm against the prompts the triage let through untouched, plus `analyses_never_logged` when nonzero (nudged, never whispered, in neither arm). When its `verdict` says there is not enough data, say that and stop - do not read a delta off a handful of runs. Then correction rate, follow-up turns, what the causes say, and `trend` if it has enough to speak. |
